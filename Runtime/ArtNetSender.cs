...
namespace UltraCombos.ArtNet
{
	...
	public class ArtNetSender : MonoBehaviour
	{
		...
		{
			Debug.unityLogger.Log( type, $"<b>[ArtNet Sender]</b> {msg}" );
		}
		...
		public void Set(ushort universe, int channel, byte value, int physical = 0)
		{
			...
			if ( found == null )
			{
				Log( $"Can't find universe {universe}", LogType.Error );
				return;
			}
			if ( channel < 1 || channel > 512 )
			{
				Log( "Channel is out of bound. " + channel, LogType.Error );
			}
		...
		public void Trigger(ushort universe, int channel)
		{
			...
			if ( channel < 1 || channel > 512 )
			{
				Log( $"Channel {channel} is out of bound. ", LogType.Error );
			}
