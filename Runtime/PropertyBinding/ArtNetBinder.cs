...
namespace UltraCombos.ArtNet
{
	...
	{
		public ArtNetReceiver m_Receiver;
		[Range( 0, 512 )]
		public short m_Universe = 0;
		[Range( 1, 512 )]
		...
		{
			m_Receiver.onDataUpdated.AddListener( OnDataUpdated );
		}

		protected virtual void OnDisable()
		{
			m_Receiver.onDataUpdated.RemoveListener( OnDataUpdated );
		}
		...
		{
			if ( m_Receiver.Data.ContainsKey( m_Universe ) && IsValid )
			{
				UpdateBinding( m_Receiver.Data[m_Universe].data );
			}
		}
		...
		private void OnNewPacket(NewPacketEventArgs<ArtNetPacket> args)
		{
			...
			if ( dmx.Universe != m_Universe || IsValid == false )
				return;

			UpdateBinding( dmx.DmxData );
		}
